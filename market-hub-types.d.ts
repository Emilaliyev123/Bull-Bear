...
export interface StrategyBreakdown {
  ...
  notes?: string[];
  newsCalendar?: NewsCalendarBreakdown;
}

// --- News & Economic Calendar Risk Engine types ---

export type CalendarDataStatus = "static-calendar" | "live-calendar" | "not-connected";
...
export interface NormalizedEvent {
  ...
  importance: string;
  source: string;
  sourceStatus: CalendarDataStatus;
  fetchedAt: string;
}

export interface NewsRiskActiveEvent {
  id: string;
  title: string;
  category: string;
  scheduledTime: string;
...
export interface NewsRiskResult {
  level: "extreme" | "elevated" | "normal" | "unknown";
  event: string;
  scoreImpact: number;
  shouldBlockTrade: boolean;
  activeEvents: NewsRiskActiveEvent[];
  upcomingEvents: NewsRiskActiveEvent[];
  explanation: string;
  dataStatus: CalendarDataStatus;
}

export interface NewsCalendarBreakdown {
  dataStatus: CalendarDataStatus;
  activeEvents: NewsRiskActiveEvent[];
  upcomingEvents: NewsRiskActiveEvent[];
  explanation: string;
